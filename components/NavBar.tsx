  ...
  return (
    <nav className="fixed top-0 w-full z-50 glass-nav transition-colors duration-300 border-b border-outline-variant/10">
      <div className="max-w-screen-2xl mx-auto flex justify-between items-center px-8 py-4">
        <Link href="/" className="text-2xl font-headline font-bold text-rose-900">
          개발자의 도서관
        </Link>
      ...
      {menuOpen && (
        <div className="md:hidden bg-surface-container-low px-6 pb-6 space-y-1 font-headline font-bold text-sm animate-slide-down">
          <Link
            href="/tags"
            className="block py-2.5 text-on-surface-variant hover:text-rose-700 border-b border-outline-variant/10"
          ...
          <Link
            href="/search"
            className="block py-2.5 text-on-surface-variant hover:text-rose-700"
            onClick={() => setMenuOpen(false)}
          >
            Search
          </Link>
