...
~~8. `==`와 `equals()`의 차이를 제대로 이해하지 못하면 어떤 버그가 생길까?~~ `작성완료`
...
~~13. Spring에서 순환 참조가 문제가 되는 이유는 무엇일까?~~ `작성완료`
...
~~20. REST API에서 `PUT`과 `PATCH`는 실제 서비스 설계에서 어떻게 다르게 써야 할까?~~ `작성완료`
...
~~55. Java Thread Pool에서 core size, max size, queue size를 잘못 잊으면 어떤 장애가 생길까?~~ `작성완료`
...
~~62. 트랜잭션 격리 수준을 낙추거나 높이면 동시성과 정합성에 어떤 변화가 생길까?~~ `작성완료`
...
~~70. 로그, 메트릭, 트레이스는 장애 분석에서 각각 어떤 질문에 답해줌 수 있을까?~~ `작성완료`
## Backend / JVM / Spring
...
~~356. Netty 같은 event-driven 서버에서 blocking call 하나가 전체 처리량에 어떤 영향을 줄 수 있을까?~~ `작성완료`
~~357. Spring WebFlux를 쓰면 자동으로 성능이 좋아지는가, blocking dependency가 남아 있으면 어떤 일이 생길까?~~ `작성완료`
~~358. servlet thread-per-request 모델과 reactive 모델은 backpressure를 다루는 방식이 어떻게 다를까?~~ `작성완료`
~~359. Spring proxy 기반 AOP와 AspectJ weaving은 적용 시점과 제약이 어떻게 다를까?~~ `작성완료`
