...
~~139. binary search는 정렬된 배열에서만 쓰는 기법일까, 답의 범위를 줄이는 문제에도 적용될 수 있을까?~~ `작성완료`
~~140. two pointer와 sliding window는 비슷해 보이지만 어떤 조건에서 각각 더 자연스럽게 쓰일까?~~ `작성완료`
...
~~144. dynamic programming은 단순히 점화식을 세우는 문제가 아니라, 상태 정의와 중복 구조를 찾는 문제라고 볼 수 있을까?~~ `작성완료`
