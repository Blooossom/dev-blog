...
~~104. TCP는 신뢰성을 보장한다는데, 애플리케이션 레벨에서 timeout과 retry가 여전히 필요한 이유는 무엇일까?~~ `작성완료`
~~105. UDP는 신뢰성이 없다는 설명만으로 충분할까, 어떤 상황에서는 UDP가 TCP보다 더 적합할까?~~ `작성완료`
~~106. HTTP/1.1의 keep-alive와 HTTP/2의 multiplexing은 연결 사용 방식에서 무엇이 근본적으로 다를까?~~ `작성완료`
~~107. HTTP/2 multiplexing이 있어도 head-of-line blocking 문제가 완전히 사라지지 않는 이유는 무엇일까?~~ `작성완료`
~~108. HTTP/3가 TCP 대신 QUIC 위에서 동작하면서 얻는 이점은 무엇일까?~~ `작성완료`
...
~~113. 프록시, 리버스 프록시, 로드밸런서는 트래픽 흐름에서 각각 어떤 위치와 체임을 가질까?~~ `작성완료`
...
397. tail latency는 평균 latency보다 왕 사용자 경험과 capacity planning에 더 중요한 지표가 될 수 있을까?
