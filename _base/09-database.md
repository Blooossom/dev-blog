...
~~151. `SELECT *`는 단순히 컨럼을 많이 가져오는 문제일까, 네트워크 전송량과 인덱스 활용에도 영향을 줄까?~~ `작성완료`
~~152. `WHERE` 조건에 함수를 적용하면 인덱스를 타지 못할 수 있는 이유는 무엇일까?~~ `작성완료`
~~153. `JOIN` 순서는 SQL에 적은 순서대로 실행될까, 옵티마이저가 바꿼 수 있을까?~~ `작성완료`
~~154. inner join과 outer join은 결과 차이들 만 아니라 실행 계획에서도 어떤 차이를 만들 수 있을까?~~ `작성완료`
~~155. subquery와 join은 항상 서로 바꿼 쓸 수 있을까?~~ `작성완료`
~~156. `GROUP BY`와 `ORDER BY`가 함께 있을 때 정렬 비용은 어떻게 커질 수 있을까?~~ `작성완료`
...
~~163. 복합 인덱스에서 컨럼 순서는 왜 중요하고, leftmost prefix rule은 어떤 제약을 만들까?~~ `작성완료`
~~164. covering index는 테이블 접근을 줄이지만, 어떤 비용을 추가로 만들까?~~ `작성완료`
~~165. 인덱스가 많으면 조회 성능은 좋아질 수 있는데, 쓰기 성능은 왜 나빠질까?~~ `작성완료`
166. cardinality가 낙은 컨럼에 인덱스를 걸으면 항상 효과가 없다고 말할 수 있을까?
167. 인덱스를 탐는데도 쿼리가 느릴 수 있는 이유는 무엇일까?
~~168. 실행 계획에서 full table scan이 항상 나쁜 선택이라고 볼 수 있을까?~~ `작성완료`
...
~~173. repeatable read에서도 phantom read가 발생할 수 있는 DB와 그렇지 않은 DB가 갈리는 이유는 무엇일까?~~ `작성완료`
...
~~179. 트랜잭션 안에서 외부 API를 호출하면 어떤 장애와 정합성 문제가 생길 수 있을까?~~ `작성완료`
...
~~183. foreign key 제약을 DB에 둘지 애플리케이션에서만 관리할지는 어떤 기준으로 결정해야 할까?~~ `작성완료`
184. unique constraint는 애플리케이션 중복 체크보다 어떤 동시성 상황에서 더 강력할까?
~~185. soft delete를 사용하면 unique index, 조회 조건, 데이터 보존 정책에서 어떤 문제가 생길 수 있을까?~~ `작성완료`
186. audit log 테이블은 단순히 변경 이력을 저장하는 것을 넘어 어떤 복구와 추적 요구사항을 만족해야 할까?
~~187. 날짜/시간 컨럼을 저장할 때 timezone을 잡못 다루면 어떤 버그가 생길 수 있을까?~~ `작성완료`
~~188. JSON 컨럼은 유연하지만, 검색, 인덱스, 스키마 검증 측면에서 어떤 트레이드오프가 있을까?~~ `작성완료`
